...
func(s KnowledgeStore)CreateFileAsset(ctx context.Context,input knowledge.CreateFileAssetInput)(knowledge.FileAsset,error){item:=knowledge.FileAsset{ID:newID("fil"),ProjectID:input.ProjectID,Name:input.Name,Extension:input.Extension,MIMEType:input.MIMEType,Size:input.Size,Status:input.Status,StoragePath:input.StoragePath,SourceID:input.SourceID,CreatedAt:time.Now().UTC()};_,err:=s.DB.ExecContext(ctx,`INSERT INTO knowledge_files(id,project_id,name,extension,mime_type,size_bytes,status,storage_path,source_id,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)`,item.ID,item.ProjectID,item.Name,item.Extension,item.MIMEType,item.Size,item.Status,item.StoragePath,nullable(item.SourceID),item.CreatedAt);return item,err}
func(s KnowledgeStore)ListFileAssets(ctx context.Context,projectID string)([]knowledge.FileAsset,error){rows,err:=s.DB.QueryContext(ctx,`SELECT id,project_id,name,extension,mime_type,size_bytes,status,storage_path,COALESCE(source_id,''),created_at FROM knowledge_files WHERE (?='' OR project_id=?) ORDER BY created_at DESC`,projectID,projectID);if err!=nil{return nil,err};defer rows.Close();items:=[]knowledge.FileAsset{};for rows.Next(){var item knowledge.FileAsset;if err:=rows.Scan(&item.ID,&item.ProjectID,&item.Name,&item.Extension,&item.MIMEType,&item.Size,&item.Status,&item.StoragePath,&item.SourceID,&item.CreatedAt);err!=nil{return nil,err};items=append(items,item)};return items,rows.Err()}
func(s KnowledgeStore)GetFileAsset(ctx context.Context,id string)(knowledge.FileAsset,error){var item knowledge.FileAsset;err:=s.DB.QueryRowContext(ctx,`SELECT id,project_id,name,extension,mime_type,size_bytes,status,storage_path,COALESCE(source_id,''),created_at FROM knowledge_files WHERE id=?`,id).Scan(&item.ID,&item.ProjectID,&item.Name,&item.Extension,&item.MIMEType,&item.Size,&item.Status,&item.StoragePath,&item.SourceID,&item.CreatedAt);if errors.Is(err,sql.ErrNoRows){return knowledge.FileAsset{},knowledge.ErrNotFound};return item,err}
func(s KnowledgeStore)DeleteFileAsset(ctx context.Context,id string)(knowledge.FileAsset,error){item,err:=s.GetFileAsset(ctx,id);if err!=nil{return knowledge.FileAsset{},err};_,err=s.DB.ExecContext(ctx,`DELETE FROM knowledge_files WHERE id=?`,id);return item,err}
