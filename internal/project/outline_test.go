...
func TestParseOutlineNormalizesHeadingLevels(t *testing.T) {
	...
	if len(items) != 3 || items[0].NodeType != "VOLUME" || items[1].NodeType != "CHAPTER" || items[2].Level != 3 { t.Fatalf("unexpected items: %#v", items) }
}
...
func TestParseOutlineSupportsTechnicalDocuments(t *testing.T) {
	...
	if err != nil || items[0].NodeType != "MODULE" || items[1].NodeType != "SECTION" { t.Fatalf("unexpected items: %#v, %v", items, err) }
}
