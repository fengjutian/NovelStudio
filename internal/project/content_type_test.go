...
func TestContentTypeCRUDAndProjectValidation(t *testing.T) {
	...
	created, err := store.CreateContentType(ctx, project.CreateContentTypeInput{Code: "MARKETING_COPY", Name: "营销文案", Icon: "营", Accent: "rose", Description: "广告与活动文案"})
	if err != nil {
		t.Fatal(err)
	}
	...
	updated, err := store.UpdateContentType(ctx, created.Code, project.UpdateContentTypeInput{Name: "品牌文案", Icon: "牌", Accent: "violet", Description: "品牌内容"})
	if err != nil || updated.Name != "品牌文案" {
