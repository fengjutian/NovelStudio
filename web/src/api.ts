...
export const api = {
  ...
  batchGenerate: (projectId:string,input:{nodeIds:string[];instruction:string;knowledgeQuery:string;windowSize:number;force?:boolean}) => request<AITask<GenerationResult>>(`/api/v1/projects/${projectId}/batch-generation-tasks`,{method:'POST',body:JSON.stringify(input)}),
  ...
  updateMemory:(id:string,input:{type:MemoryEntry['type'];name:string;summary:string;status:string;attributes?:Record<string,unknown>})=>request<MemoryEntry>(`/api/v1/memories/${id}`,{method:'PUT',body:JSON.stringify(input)}),
  ...
  analyzeStorylines:(projectId:string)=>request<AITask<StorylineAnalysisResult>>(`/api/v1/projects/${projectId}/storyline-analysis-tasks`,{method:'POST'}),
