...
function Workspace({ project, contentTypes, initialTab, onClose }: { project: Project; contentTypes:ContentType[]; initialTab: WorkspaceTab; onClose: () => void }) {
  ...
  const allBatchNodes=(documentTree.data?.items??[]).filter(node=>node.documentId&&orderedDocuments.some(document=>document.id===node.documentId))
  const startWorkspaceBatch=useMutation({mutationFn:()=>{const draft=loadChapterDraft(project);const context=[draft.requirement&&`原始创作简报：\n${draft.requirement}`,draft.outline&&`全书目录：\n${draft.outline}`].filter(Boolean).join('\n\n');return api.batchGenerate(project.id,{nodeIds:selectedBatchNodeIds,instruction:`为《${project.name}》中的所选章节生成完整正文。严格遵守创作简报和完整目录，保持章节之间设定、时间线和叙述风格一致，只输出当前章节的 Markdown 正文。${context?`\n\n${context}`:''}`,knowledgeQuery:draft.knowledgeQuery,windowSize:1})},onSuccess:task=>setWorkspaceBatchTaskId(task.id)})
  const regenerateAll=useMutation({mutationFn:()=>{const draft=loadChapterDraft(project);const context=[draft.requirement&&`原始创作简报：\n${draft.requirement}`,draft.outline&&`全书目录：\n${draft.outline}`].filter(Boolean).join('\n\n');return api.batchGenerate(project.id,{nodeIds:allBatchNodes.map(node=>node.id),instruction:`重新生成《${project.name}》的全部章节正文。严格遵守创作简报和完整目录，保持章节之间设定、时间线和叙述风格一致，只输出当前章节的 Markdown 正文。${context?`\n\n${context}`:''}`,knowledgeQuery:draft.knowledgeQuery,windowSize:1,force:true})},onSuccess:task=>{setBatchDocumentIds(allBatchNodes.flatMap(node=>node.documentId?[node.documentId]:[]));setWorkspaceBatchTaskId(task.id)}})
