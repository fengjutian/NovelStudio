...
import type { ContentNode, ContentType, Document, GenerationResult, KnowledgeFile, KnowledgeSource, MemoryEntry, MemoryExtractionResult, MemorySuggestion, OutlineItem, Project, QualityGenerationResult, StorylineAnalysisResult } from './types'
...
function StorylinePanel({project}:{project:Project}){
  const queryClient=useQueryClient()
  const[name,setName]=useState('');const[summary,setSummary]=useState('');const[stage,setStage]=useState('SETUP');const[selectedId,setSelectedId]=useState('');const[analysisOpen,setAnalysisOpen]=useState(false);const[analysisTaskId,setAnalysisTaskId]=useState('')
