...
function ModelConfigPanel(){
  ...
  const test=useMutation({mutationFn:()=>api.testLocalModelConfig({provider:active,config:providerPayload(active==='minimax'?minimax:deepseek)}),onSuccess:result=>toast.success(`连接成功：${result.model} · ${result.latencyMs} ms`)})
  ...
  return <section className="model-config-panel"><div className="section-head"><div><p className="eyebrow">LOCAL PROVIDERS</p><h2>本地模型配置</h2><p>Key 仅写入 <code>{config.data?.path??'.local/model-config.json'}</code>，不会保存到数据库或返回明文。</p></div><div className="model-config-actions"><Button variant="secondary" disabled={test.isPending||config.isLoading} onClick={()=>test.mutate()}>{test.isPending?'测试中…':'测试当前配置'}</Button><Button disabled={save.isPending||config.isLoading} onClick={()=>save.mutate()}>{save.isPending?'保存中…':'保存配置'}</Button></div></div><div className="provider-grid">{providerCard('deepseek','DeepSeek',deepseek,setDeepSeek,['deepseek-v4-flash','deepseek-v4-pro'])}{providerCard('minimax','MiniMax',minimax,setMiniMax,['MiniMax-M2.7','MiniMax-M2.7-highspeed','MiniMax-M2.5'])}</div>{save.isError&&<p className="quality-error">保存失败：{save.error.message}</p>}{test.isError&&<p className="quality-error">连接失败：{test.error.message}</p>}{test.isSuccess&&<p className="model-test-success">连接成功：{test.data.model} · {test.data.latencyMs} ms</p>}<div className="local-security-note"><Database size={15}/><span>可先测试当前表单；配置保存后需重启 Go API 才会用于生成任务。</span></div></section>
}
...
function LegacyChapterDocumentGenerator({project,documents,modelReady,onOpenDocument}:{project:Project;documents:Document[];modelReady:string[];onOpenDocument:(document:Document)=>void}){
  ...
  return <div className="chapter-generator">
    ...
    <div className="generator-layout"><section className="generator-main-card">
      ...
      <section className="generator-action final"><span className="action-icon"><FileStack/></span><div><small>步骤 4</small><h3>批量生成正文</h3><p>{!canWrite?'写作模型尚未配置，暂时无法生成正文。':nodeIds.length?`将以 2 个章节为一组依次生成，共 ${nodeIds.length} 篇。`:'确认目录后即可开始批量生成。'}</p></div><button className="primary generate-documents" disabled={!nodeIds.length||generateDocuments.isPending||Boolean(batchRunning)||!canWrite} onClick={()=>generateDocuments.mutate()}>{batchRunning?`正在生成 · ${batchTask.data?.progress??0}%`:`生成 ${nodeIds.length||0} 篇文档`}<ArrowRight size={15}/></button>{batchRunning&&<div className="generator-progress"><i><b style={{width:`${batchTask.data?.progress??0}%`}}/></i><span>{batchTask.data?.message}</span></div>}{(generateDocuments.isError||batchTask.data?.status==='FAILED')&&<p className="quality-error">{generateDocuments.error?.message||batchTask.data?.error}</p>}</section>
