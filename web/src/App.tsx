...
function StorylinePanel({project}:{project:Project}){
  const queryClient=useQueryClient()
  const[name,setName]=useState('');const[summary,setSummary]=useState('');const[stage,setStage]=useState('SETUP');const[selectedId,setSelectedId]=useState('');const[analysisOpen,setAnalysisOpen]=useState(false);const[documentId,setDocumentId]=useState('');const[analysisTaskId,setAnalysisTaskId]=useState('')
  ...
  const documents=useQuery({queryKey:['documents',project.id],queryFn:()=>api.documents(project.id)})
  const analysisTask=useQuery({queryKey:['storyline-analysis',analysisTaskId],queryFn:()=>api.task<MemoryExtractionResult>(analysisTaskId),enabled:Boolean(analysisTaskId),refetchInterval:query=>['SUCCESS','FAILED','CANCELLED'].includes(query.state.data?.status??'')?false:700})
  const analyze=useMutation({mutationFn:()=>api.extractMemories(project.id,documentId),onSuccess:task=>setAnalysisTaskId(task.id)})
  const candidates=(analysisTask.data?.result?.memories??[]).filter(item=>item.type==='PLOT')
  const clearForm=()=>{setSelectedId('');setName('');setSummary('');setStage('SETUP')}
  const save=useMutation({mutationFn:()=>selectedId?api.updateMemory(selectedId,{type:'PLOT',name:name.trim(),summary:summary.trim(),status:'ACTIVE',attributes:{...(storylines.data?.items.find(item=>item.id===selectedId)?.attributes??{}),kind:'STORYLINE',stage}}):api.createMemory(project.id,{type:'PLOT',name:name.trim(),summary:summary.trim(),status:'ACTIVE',attributes:{kind:'STORYLINE',stage}}),onSuccess:()=>{queryClient.invalidateQueries({queryKey:['memories',project.id]});toast.success(selectedId?'故事线已更新':'故事线已添加');clearForm()}})
  ...
  const remove=useMutation({mutationFn:api.deleteMemory,onSuccess:()=>{queryClient.invalidateQueries({queryKey:['memories',project.id]});clearForm()}})
  ...
  const selectItem=(id:string)=>{const item=ordered.find(value=>value.id===id);if(!item)return;setSelectedId(item.id);setName(item.name);setSummary(item.summary);setStage(String(item.attributes?.stage??'DEVELOPMENT'))}
  return <><div className="workspace-title storyline-title"><div><p className="eyebrow">STORYLINE</p><h2>故事线</h2><p>分析已有正文，在可视化画布中梳理并修改叙事推进。</p></div><div><button className="secondary" onClick={()=>setAnalysisOpen(true)}><Sparkles size={14}/> 分析现有故事线</button><span>{storylines.data?.total??0} 条故事线</span></div></div><div className="storyline-layout"><form className="studio-form storyline-form" onSubmit={event=>{event.preventDefault();save.mutate()}}><div><p className="eyebrow">{selectedId?'EDIT THREAD':'NEW THREAD'}</p><h3>{selectedId?'修改故事线':'添加故事线'}</h3><small>{selectedId?'正在编辑画布中选中的节点。':'记录一条贯穿多个章节的叙事目标。'}</small></div><label>故事线名称<input required maxLength={120} value={name} onChange={event=>setName(event.target.value)} placeholder="例如：复制人追寻身份真相"/></label><label>当前阶段<select value={stage} onChange={event=>setStage(event.target.value)}>{Object.entries(stageLabels).map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label><label>目标与关键冲突<textarea required rows={6} maxLength={1000} value={summary} onChange={event=>setSummary(event.target.value)} placeholder="描述起点、核心冲突、关键转折与预期落点…"/></label>{save.isError&&<p className="quality-error">{save.error.message}</p>}<button className="primary" disabled={save.isPending||!name.trim()||!summary.trim()}>{selectedId?<Pencil size={15}/>:<Plus size={15}/>} {save.isPending?'正在保存…':selectedId?'保存修改':'添加故事线'}</button>{selectedId&&<div className="storyline-form-actions"><button type="button" className="secondary" onClick={clearForm}>取消编辑</button><button type="button" className="storyline-delete" onClick={()=>remove.mutate(selectedId)}><Trash2 size={14}/> 删除</button></div>}</form><section className="storyline-flow">{storylines.isLoading?<p className="empty-line">正在加载故事线…</p>:nodes.length?<ReactFlow nodes={nodes} edges={edges} fitView fitViewOptions={{padding:.2}} nodesDraggable nodesConnectable={false} elementsSelectable onNodeClick={(_,node)=>selectItem(node.id)}><Background color="#d9cfc1" gap={22}/><MiniMap pannable zoomable nodeColor="#c98a48" maskColor="rgba(247,243,237,.72)"/><Controls showInteractive={false}/></ReactFlow>:<div className="storyline-empty"><Sparkles/><strong>还没有故事线</strong><span>手动添加，或让 AI 从已有正文中分析。</span></div>}</section></div><Dialog open={analysisOpen} onOpenChange={setAnalysisOpen}><DialogContent className="dialog storyline-analysis-dialog"><DialogHeader><p className="eyebrow">AI STORY ANALYSIS</p><DialogTitle>分析现有故事线</DialogTitle><DialogDescription>选择一篇已有正文，AI 将识别其中的剧情线；由你确认后再加入画布。</DialogDescription></DialogHeader><label>来源文档<select value={documentId} onChange={event=>{setDocumentId(event.target.value);setAnalysisTaskId('')}}><option value="">请选择文档</option>{documents.data?.items.map(item=><option value={item.id} key={item.id}>{item.title}</option>)}</select></label><button className="primary" disabled={!documentId||analyze.isPending||analysisTask.data?.status==='RUNNING'} onClick={()=>analyze.mutate()}><Sparkles size={14}/>{analysisTask.data?.status==='RUNNING'?`分析中 ${analysisTask.data.progress}%`:'开始分析'}</button>{(analyze.isError||analysisTask.data?.status==='FAILED')&&<p className="quality-error">{analyze.error?.message||analysisTask.data?.error}</p>}{analysisTask.data?.status==='SUCCESS'&&<div className="storyline-candidates">{candidates.map((item,index)=><article key={`${item.name}-${index}`}><div><strong>{item.name}</strong><p>{item.summary}</p></div><button className="secondary" disabled={addCandidate.isPending} onClick={()=>addCandidate.mutate(item)}>加入画布</button></article>)}{candidates.length===0&&<p className="empty-line">这篇文档中没有识别到明确的故事线。</p>}</div>}</DialogContent></Dialog></>
}
