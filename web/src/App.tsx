...
function StorylinePanel({project}:{project:Project}){
  ...
  const save=useMutation({mutationFn:()=>api.createMemory(project.id,{type:'PLOT',name:name.trim(),summary:summary.trim(),status:'ACTIVE',attributes:{kind:'STORYLINE',stage}}),onSuccess:()=>{queryClient.invalidateQueries({queryKey:['memories',project.id]});toast.success('故事线已添加');clearForm()}})
